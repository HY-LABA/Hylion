...
- **USB-UART 칩**: Silicon Labs CP2102 (`idVendor=10c4 idProduct=ea60`)
  - 근거: [1. Tutorials/Program Files /6.ROS2 Application Program/wit_ros2_imu/imu_usb.rules](1.%20Tutorials/Program%20Files%20/6.ROS2%20Application%20Program/wit_ros2_imu/imu_usb.rules)
...
| Quat 순서 | (Q0,Q1,Q2,Q3) 를 (w,x,y,z) 가정 ([imu.py:272-276](../bhl/Berkeley-Humanoid-Lite-Lowlevel-main/berkeley_humanoid_lite_lowlevel/robot/imu.py#L272-L276)) | 펌웨어 따라 다름, **검증 필요** | 실측 검증 (§5.Step 5) |
...
```
# /etc/udev/rules.d/99-hylion-imu.rules
KERNEL=="ttyUSB*", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", SYMLINK+="hylion_imu"
```

> **주의**: CAN-USB 어댑터도 같은 CP210x 칩이면 `idVendor/Product` 가 겹침. `lsusb -v` 로 시리얼 확인 후 필요시 `ATTRS{serial}=="..."` 로 구분.
...
```bash
lsusb | grep -i "10c4\|cp210"
# 예시: Bus 003 Device 005: ID 10c4:ea60 Silicon Labs CP210x UART Bridge
