...
## 6) Note on Reply Audio File Storage

Reply audio generated by speak_with_lipsync is stored in OS temp folder under hylion_tts.

- On Ubuntu/Jetson: typically /tmp/hylion_tts
- File pattern: reply_<timestamp>.mp3 (when gTTS available) or reply_<timestamp>.wav (fallback)
