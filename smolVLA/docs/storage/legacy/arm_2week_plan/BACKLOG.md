...
| 6 | `docs/lerobot_study/07c_smolvla_base_test_results.md` 작성 — TODO-06b 에서 §1~§6, TODO-07b 에서 §7(7.1~7.5) 완성 (2026-04-29). 04 시작 시점에 `docs/storage/` 에서 `docs/lerobot_study/` 로 이동·rename (2026-04-30) — 03/07 시리즈 (07/07b/07c) 정렬 일관성. **2026-05-12 docs 정리 시 파일 삭제됨.** | TODO-05 / TODO-06b / TODO-07b | 높음 | 완료 (2026-04-29) → 파일 삭제 (2026-05-12) |
...
| 3 | `orin/config/ports.json`, `cameras.json` 의 git 추적 vs gitignore 정책 결정 — TODO-O1 §1-2 에서 git 추적 가정. 단 사용자 환경 의존(시연장 셋업 후 안정적) 이라 별 인스턴스 환경에서 충돌 가능. 정책 명문화 필요 | TODO-O1 study | 낮음 | 완료 (07 W4 정책 문서 신규 — `docs/storage/10_orin_config_policy.md`, 2026-05-03; 2026-05-07 재번호 구 15→10). **2026-05-14 문서 삭제 (fresh start — 새 계획 수립 시 정책 재논의)** |
...
| 7 | [ad-hoc] `orin/inference/hil_inference.py` 의 `TASK` / `MODEL_ID` 상수 인자화 — 사이클별 fine-tune 모델마다 별도 스크립트 (lego_v1_inference.py 등) 작성 중복. `--task` / `--model-id` 인자로 통합 가능 (현재 `--model-id` 만 지원). 인자화 후 lego_v1_inference.py 의 차이는 default 값과 카메라 키 alias (overview/wrist) 만 남게 됨 — 통합 vs 별도 유지 결정 함께. | 2026-05-06 ad-hoc lego_v1_inference 작성 후속 | 중간 (다음 fine-tune 사이클 트리거) | 미완 |
| 8 | [ad-hoc] Orin venv (`~/smolvla/orin/.hylion_arm`) 의 `HF_HOME=/home/laba/smolvla/.hf_cache` 영구 export — 현재 매번 수동 export 필요. `orin/scripts/setup_env.sh` 의 venv activate 후크 또는 `.hylion_arm/bin/activate` 말미에 export 추가. DGX 와 형제 구조 정합 (DGX 측 venv 는 `dgx/scripts/setup_env.sh` 가 동일 처리 — 확인 필요). Coupled File Rule 1 (setup_env.sh 변경 시 02_orin_pyproject_diff.md 갱신) 적용 검토. | 2026-05-06 ad-hoc lego_v1_inference 실행 절차 | 중간 (다음 Orin 추론 작업 마찰) | 미완 |
