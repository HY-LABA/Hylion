...
6. **배포 스크립트와의 충돌**
   - `scripts/deploy_orin.sh`는 호스트 파일시스템(`~/smolvla/`) 대상 rsync 구조 ([README.md:85-90](README.md#L85-L90))
...
- [x] **Orin에 wheel 1회 수동 설치** (2026-04-23 완료)
  - `.venv`에 한 번 설치되면 영구 유지 — `scripts/deploy_orin.sh`가 `.venv`를 건드리지 않으므로
    재배포해도 재설치 불필요. wheel을 `scripts/deploy_orin.sh` rsync 범위에 넣지 않는다.
  - devPC에서:
    ```bash
