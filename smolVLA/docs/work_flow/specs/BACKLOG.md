...
| 13 | [ad-hoc] `orin/scripts/run_inference_leftarm_v2.sh` line 254 `--max-steps 50` 을 Orin 에서 *직접 sed* 로 500 → 1000 으로 임시 변경 (시연장 단축 trial). devPC 의 wrapper 는 *기존 500 그대로* — Orin ↔ devPC 비동기 상태. 다음 사이클에서 *학습 방법 점검 후 적정 max-steps 결정* 시 정식 동기화 (env 패턴 또는 default 값 정합). | 2026-05-18 시연장 ad-hoc | 중간 | 완료 (2026-05-18 — camera_empty 추론 검증 시 사용자가 시연장에서 50 → 1000 재변경 → 본 사이클 Orin 점검 시 Orin 정본 1000 으로 devPC sync. 정식 동기화 완료. env override 패턴은 향후 사이클에서) |
| 15 | [Orin 점검 부산물] `orin/interactive_cli/__pycache__/` 옛 era 잔존 (12KB, .py 원본 없음 = c512eae "refactor(storage): interactive_cli legacy 이관" commit 시 .py 만 git 에서 삭제 → 다음 deploy_orin 의 rsync `--exclude '__pycache__'` 때문에 __pycache__ 만 Orin 에 잔존). 본 사이클 ssh rm -rf 로 정리. 시스템 패턴 — *legacy 이관 commit 시 동시에 prod 환경 __pycache__ 정리도 의무화* 검토. | 2026-05-18 Orin 점검 ad-hoc | 낮음 | 완료 (즉시 정리). 시스템 패턴은 reflection 단계. |
