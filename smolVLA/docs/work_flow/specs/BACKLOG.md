...
| 6 | `docs/storage/07_smolvla_base_test_results.md` 작성 — TODO-06b 에서 §1~§6, TODO-07b 에서 §7(7.1~7.5) 완성 (2026-04-29). | TODO-05 / TODO-06b / TODO-07b | 높음 | 완료 (2026-04-29) |
...
| 13 | `num_steps=5` vs `num_steps=10` latency 미감소 — 실측 p50 동일(5.59ms). TODO-07b 실 카메라 입력에서 재측정 예정이었으나 latency 정량 재측정 미수행. 04_leftarmVLA 학습 후 실 제어 루프에서 재확인 | TODO-06b PARTIAL | 중간 | 미완 |
...
| 16 | wrist 카메라 방향 플립 필요 — `--flip-cameras wrist` 없이 실행 시 wrist 카메라 이미지가 뒤집힘. 04_leftarmVLA 학습 전 사전학습 분포의 wrist 카메라 방향과 일치하는지 확인 후 hil_inference.py 기본값 또는 데이터 수집 스크립트에 반영 필요 | TODO-07b prod 검증 | 높음 | 미완 |
| 17 | 팔 calibration 중간값 경직 — live 50 step 이후 팔이 calibration 중간값 근처에서 멈추고 경직됨. 사전학습 분포(lego 큐브 pick-and-place, 특정 책상 배치) 와 본 환경 차이의 예상 동작. 03 마일스톤 범위 내 정상 결과. 04_leftarmVLA 학습 및 fine-tuning 으로 해결 필요 | TODO-07b prod 검증 | 중간 | 04_leftarmVLA 이관 |
