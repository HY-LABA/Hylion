...
```
대상: prof_computer/docs/orin_a2_eval_2026-05-17.md (문서 파일 — 코드 단위 테스트 없음)
...
| (c) 성능평가 시트 신설 (20 trial, task×orientation, 성공/실패, 실패 원인) | ✅ | `prof_computer/docs/orin_a2_eval_2026-05-17.md` 신규 — 구조 정합 |
...
| # | 위치 | 권장 |
|---|---|---|
| 1 | `prof_computer/README.md` 트리 (line 58-59) | `orin_a2_eval_2026-05-17.md` 신규 파일이 트리에 미등록. Coupled Rules §6 엄밀 적용 시 본문 갱신 대상. 단 task-executor 가 "단순 파일 추가 — BACKLOG 권장"으로 명시 결정했고, 기존 `after_run_checklist.md` 도 동일하게 미등록 상태(기존 drift). BACKLOG 등록 권장. |
...
| Coupled File Rules §6 | ⚠️ | `prof_computer/README.md` 트리에 `orin_a2_eval_2026-05-17.md` 미등록. 엄밀하게는 §6 갱신 의무 대상이나, task-executor 가 BACKLOG 처리로 결정 + 기존 drift 파일(after_run_checklist.md)도 동일 미등록 상태 → 기존 drift 연장 판단. Recommended로 분류 (Critical 상향 불요). |
