...
TODO-O1 의 산출물 (`docs/storage/08_orin_structure.md` §5 마이그레이션 계획) 을 그대로 실행. 03 산출물 5개 (`smoke_test.py`, `inference_baseline.py`, `measure_latency.py`, `load_checkpoint_test.py`, `hil_inference.py`) 의 동작 회귀 없이 새 구조 (`tests/`, `config/`, `checkpoints/` 신규 + `calibration/` 제거 + entrypoint 2개 제거 + run_teleoperate.sh 임시 보관) 로 변경.
...
1. `docs/storage/08_orin_structure.md` §5 마이그레이션 계획 — 본 TODO 의 입력 사양
...
- `docs/storage/08_orin_structure.md` — TODO-O1 산출물 (특히 §5 마이그레이션 계획)
