...
#   - lerobot [hardware,feetech] extra 설치 (dgx/scripts/setup_finetune_env.sh §3-c 완료 후)
...
#    NOTE: lerobot [hardware] extra 설치 필요 (setup_finetune_env.sh §3-c 완료 후 동작)
# ---------------------------------------------------------------------------
step_cameras() {
    [[ "$QUIET" == "false" ]] && echo ""
...
PYEOF
    ...
    if [[ $cam_exit -eq 3 ]]; then
        # lerobot [hardware] extra 미설치 시 — setup_finetune_env.sh §3-c 실행 전 단계에서 발생 가능
...
" "${total_exit}" "${PASS_COUNT}" "${FAIL_COUNT}" "${TMP_RESULT_JSON}"
    ...
    if [[ $FAIL_COUNT -eq 0 ]]; then
        ...
        echo "   cameras FAIL (import) → setup_finetune_env.sh §3-c 실행 후 재시도 (lerobot hardware extra 필요)"
    fi
