...
OUTPUT_DIR="${DGX_DIR}/outputs/smoke_test/$(date +%Y-%m-%d_%H-%M-%S)"

# ── 1. preflight (smoke 시나리오) ──────────────────────────────────────────────
...
# 백그라운드에서 5초 간격으로 nvidia-smi / free -h 샘플링
SAMPLE_LOG="${OUTPUT_DIR}/resource_samples.log"
mkdir -p "${OUTPUT_DIR}"
...
(
    while true; do
        ...
        free -h | grep -E "Mem|Swap" >> "${SAMPLE_LOG}"
        echo "" >> "${SAMPLE_LOG}"
        sleep 5
    done
) &
...
if [ -f "${SAMPLE_LOG}" ]; then
    ...
    PEAK_MEM=$(awk -F',' '/MiB/ {gsub(" MiB",""); if($2>max)max=$2} END {print max}' "${SAMPLE_LOG}")
