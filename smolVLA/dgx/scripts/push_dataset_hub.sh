...
#   - data_root 예시 경로: ~/smolvla/datacollector/data/ → ~/smolvla/dgx/data/
#   - heredoc 환경변수 보안 패턴 (single-quote 'PYEOF') 그대로 보존
#   - LeRobotDataset.push_to_hub 호출 인자: 원본 그대로
#
# 실행 위치: DGX 머신 (직접 실행) 또는 devPC 에서 ssh 경유
#
# 사용:
#   bash dgx/scripts/push_dataset_hub.sh \
...
#
# 결정 근거: 06_dgx_absorbs_datacollector spec 결정 H-(b) / X2 transfer.py
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo ""
            echo "필수 인자:"
            echo "  --dataset <path>     로컬 dataset 경로 (예: ~/smolvla/dgx/data/my_dataset)"
...
if [ -z "${DATASET_PATH}" ]; then
    ...
    echo "  사용 예: bash $0 --dataset ~/smolvla/dgx/data/my_dataset --repo-id myuser/my_dataset"
    exit 1
fi
...
try:
    ...
    print("    source ~/smolvla/dgx/.arm_finetune/bin/activate")
    sys.exit(1)
