#!/usr/bin/env python3
"""prof_computer/finetune/leftarm_v2/run_train.py — leftarm_v2 학습 래퍼.

DGX 의 dgx/finetune/leftarm_v2/run_train.py 의 PC 적응판. 차이:
  - --pass smoke 추가 — train_config_smoke.yaml 로드 (steps=100, save_freq=50)
  - run_name 에 _pc_ 접두 — wandb 에서 DGX run 과 식별 분리
  - video_backend 인자를 train_config 의 video_backend 키에서 읽어 전달
  - output_dir 부모 디렉터리 자동 mkdir -p (base_config.paths.output_root)

config/{base,train,train_smoke}_config.yaml → lerobot-train 명령 구성·실행.
...
사용:
  ...
  python run_train.py train --pass 2b              # 2B: 200ep 전체

학습 산출: ~/prof_computer_runs/<run_name>/  (base_config.paths.output_root)
결정 근거: dgx/docs/finetune/leftarm_v2/model_config.md (DGX 와 동일 방법론)
"""
import argparse
import os
import shlex
...
def main():
    ...
    # smoke 는 train_config_smoke.yaml, 나머지는 train_config.yaml
