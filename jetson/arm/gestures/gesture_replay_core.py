...
def connect_arm(port: str = DEFAULT_PORT, robot_id: str = DEFAULT_ID):
    ...
    NOTE: gripper 모터(ID 6)가 write 후 status packet 을 보내지 않아 lerobot 의
    writeTxRx 가 COMM_RX_TIMEOUT 을 반환한다. robot.bus._write 를 패치해
    ID 6 에 한해 raise_on_error=False 로 우회한다 (실제 write 는 정상 동작 확인됨,
    2026-06-02 직접 측정).
    """
    import types
    ...
    # gripper(ID 6) write no-status-packet 우회 패치
    _orig_write = robot.bus._write.__func__

    def _patched_write(self, addr, length, motor_id, value, *,
                       num_retry=0, raise_on_error=True, err_msg=""):
        if motor_id == 6 and raise_on_error:
            raise_on_error = False
        return _orig_write(self, addr, length, motor_id, value,
                           num_retry=num_retry, raise_on_error=raise_on_error, err_msg=err_msg)

    robot.bus._write = types.MethodType(_patched_write, robot.bus)
